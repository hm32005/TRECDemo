...
public class BatchSearchBoolQuery {

	private BatchSearchBoolQuery() {}
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			StringTokenizer st = new StringTokenizer(pair[1]);
			while (st.hasMoreTokens()) {
				bq.add(parser.parse(st.nextToken()), Occur.MUST);
			}
			writer.write(doBatchSearch(in, searcher, pair[0], bq, simstring));
		}
		reader.close();
		writer.close();
	}
	...
	public static String doBatchSearch(BufferedReader in, IndexSearcher searcher, String qid, Query query, String runtag)	 
		...
		TopDocs results = searcher.search(query, searcher.getIndexReader().numDocs());
		ScoreDoc[] hits = results.scoreDocs;
		HashMap<String, String> seen = new HashMap<String, String>();
		int numTotalHits = results.totalHits;
		StringBuffer sb = new StringBuffer();
		TreeMap<Integer, Float> sortedMap = new TreeMap<Integer, Float>();
		
		int start = 0;
		int end1 = Math.min(numTotalHits, searcher.getIndexReader().numDocs());
		int end2 = Math.min(numTotalHits, 1000);
		
		for (int i = start; i < end1; i++) {
			sortedMap.put(hits[i].doc, hits[i].score);
		}
		Iterator<Entry<Integer, Float>> mergedIter = sortedMap.entrySet().iterator();
		for (int i = start; i < end2  && mergedIter.hasNext(); i++) {
